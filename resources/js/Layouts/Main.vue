              ...
              <div v-if="user" class="relative">
                ...
                <!-- Dropdown -->
                 <div 
                   v-show="show" 
                   @click="show = false"
                   class="absolute z-50 top-16 right-0 w-40 bg-slate-800 rounded-lg text-white border-slate-300 border overflow-hidden">
                    <Link 
                      as="button"
                      class="block w-full px-6 py-3 hover:bg-slate-700 text-left">Dashboard</Link>
                    <Link 
                      :href="route('logout')"
                      method="post"
                      as="button"
